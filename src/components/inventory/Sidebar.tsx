...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <nav className={`fixed inset-y-0 left-0 z-[110] w-[220px] lg:w-[200px] bg-[#0a0e12] border-r border-white/[0.07] flex flex-col transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        ...
        <div className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
          ...
          {nav.map(({ icon: Icon, label, href }) => {
            ...
            return (
              <Link key={href} href={href} onClick={() => setOpen(false)} className={`flex items-center gap-3 px-3 h-9 rounded-lg text-[13px] font-medium transition-colors ${active ? "bg-cyan-500/10 text-cyan-400" : "text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.04]"}`}>
