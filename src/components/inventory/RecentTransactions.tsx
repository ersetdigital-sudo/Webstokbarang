      ...
      <div className="divide-y divide-white/[0.04]">
        {current.map((t) => (
          <div key={t.id} className="flex items-center gap-3 px-5 py-4 hover:bg-white/[0.02] transition-colors">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${t.type === "masuk" ? "bg-teal-500/10" : "bg-orange-500/10"}`}>
              {t.type === "masuk" ? <ArrowDownLeft size={14} className="text-teal-400" /> : <ArrowUpRight size={14} className="text-orange-400" />}
            </div>
            ...
            <div className="text-right flex-shrink-0">
              <p className={`text-[13px] font-mono font-medium ${t.type === "masuk" ? "text-teal-400" : "text-orange-400"}`}>{t.type === "masuk" ? "+" : "-"}{t.qty}</p>
      ...
      <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.07]">
        ...
        <div className="flex items-center gap-1">
          ...
          {Array.from({ length: totalPages }).map((_, i) => (
            <button key={i} onClick={() => setPage(i)} className={`w-7 h-7 rounded-md flex items-center justify-center text-[11px] font-mono transition-colors ${page === i ? "bg-cyan-500/15 text-cyan-400" : "text-zinc-500 hover:bg-white/[0.05] hover:text-white"}`}>
              {i + 1}
            </button>
          ))}
